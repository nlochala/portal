...
@if($id_card->is_active)
    <button type="button" dusk="btn-new-id-card"
            class="btn btn-outline-success mr-1 mb-3" {!! \App\Helpers\ViewHelpers::onClick("/guardian/$guardian->uuid/create_id_card") !!}>
        <i class="fa fa-plus fa-plus mr-1"></i> Add New ID Card
    </button>
@endif
<button type="button" dusk="btn-update-id-card-{{ $id_card->id }}"
        class="btn btn-outline-primary mr-1 mb-3" {!! \App\Helpers\ViewHelpers::onClick("/guardian/$guardian->uuid/id_card/$id_card->uuid/update_id_card") !!}>
