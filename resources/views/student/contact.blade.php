...
@section('content')
    ...
    <h1 class="font-w400" style="text-align: center">{{ $student->person->fullName() }}'s Contact
        Information</h1>
...
-->
    ...
    @else
        ...
        @foreach($addresses as $address)
            ...
            <!-- START FORM----------------------------------------------------------------------------->
                {!! Form::model($person,['method' => 'PATCH','id' => 'email-form','url' => '/student/' . $student->uuid . '/profile/store_email']) !!}
            ...
            <!-------------------------------- Modal: Update Email Addresses END------------------------------------------->
                ...
                {!! Form::open(['files' => false, 'id' => 'phone-form','url' => '/student/' . $student->uuid . '/profile/store_phone']) !!}
            ...
            <!-------------------------------- Modal: END------------------------------------------->
                ...
                {!! Form::open(['files' => false, 'id' => 'address-form','url' => '/student/' . $student->uuid . '/profile/store_address']) !!}
            ...
            <!-------------------------------- Modal: END------------------------------------------->
                ...
                <!-------------------------------- Modal: Update Address Start------------------------------------------->
                    ...
                    {!! Form::model($address,['method' => 'PATCH','id' => 'address-update-form','url' => "/student/$student->uuid/address/$address->uuid/update_address"]) !!}
                ...
                @section('js_after')
                    {!! JsValidator::formRequest('\App\Http\Requests\StoreStudentEmailRequest','#email-form') !!}
                    {!! JsValidator::formRequest('\App\Http\Requests\StoreStudentPhoneRequest','#phone-form') !!}
                    {!! JsValidator::formRequest('\App\Http\Requests\StoreStudentAddressRequest','#address-form') !!}
                    {!! JsValidator::formRequest('\App\Http\Requests\StoreStudentAddressRequest','#address-update-form') !!}
