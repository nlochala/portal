...
@if($passport->is_active)
    <button type="button" dusk="btn-new-passport"
            class="btn btn-outline-success mr-1 mb-3" {!! \App\Helpers\ViewHelpers::onClick("/student/$student->uuid/create_passport") !!}>
        <i class="fa fa-plus fa-plus mr-1"></i> Add New Passport
    </button>
@endif

<button type="button" dusk="btn-update-passport-{{ $passport->id }}"
        class="btn btn-outline-primary mr-1 mb-3" {!! \App\Helpers\ViewHelpers::onClick("/student/$student->uuid/passport/$passport->uuid/update_passport") !!}>
...
@foreach($passport->visas as $visa)
    ...
    <!-- START FORM----------------------------------------------------------------------------->
    {!! Form::model($visa,['method' => 'PATCH','files' => true, 'id' => "visa-edit-form-$visa->id",'url' => "/student/$student->uuid/visa/$visa->uuid/update_visa"]) !!}
...
{!! Form::open(['files' => true, 'id' => "visa-form-$passport->id",'url' => "/student/$student->uuid/passport/$passport->uuid/create_visa"]) !!}
